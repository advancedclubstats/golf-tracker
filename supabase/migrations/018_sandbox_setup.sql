-- Extend the visitor sandbox to cover Setup (courses + clubs).
--
-- Setup becomes a per-visitor "staging" area: the sandbox gets its own copy of
-- the owner's courses (with holes/tees/yardages) and clubs, so visitors can edit
-- them freely without touching the owner's real reference data. Rounds/shots are
-- cloned with their course_id/tee_id remapped onto the sandbox's copies.
--
-- Everything is one statement of materialized map-CTEs (stable new ids reused
-- across the dependent inserts) — no temp tables, so it runs as the anon role.
-- Data-modifying CTEs always execute even when not referenced by the final query.
...
-- Purge now also reaps sandbox courses + clubs (their children cascade).
